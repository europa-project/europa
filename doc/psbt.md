# PSBT Howto for Europa Core

Since Europa Core 0.17, an RPC interface exists for Partially Signed Europa
...
PSBT is an interchange format for Europa transactions that are not fully signed
...
Overall, the construction of a fully signed Europa transaction goes through the
following steps:
...
- An **Extractor** produces a valid Europa transaction (in network format)
...
#### Multisig with multiple Europa Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Europa Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `europa-cli` is used.
